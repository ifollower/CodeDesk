...
#[cfg(target_os = "windows")]
const RD_DRIVER_INF_PATH: &str = "drivers/CodeDeskPrinterDriver/CodeDeskPrinterDriver.inf";

#[cfg(target_os = "windows")]
pub fn is_driver_available() -> bool {
    std::env::current_exe()
        .ok()
        .and_then(|path| path.parent().map(|parent| parent.join(RD_DRIVER_INF_PATH)))
