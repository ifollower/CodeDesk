...
#[cfg(target_os = "windows")]
impl CommandBuilder {
    /// Set the user token to use when spawning this command.
    /// This is used to spawn commands as a different user on Windows.
    pub fn set_user_token(&mut self, token: HANDLE) {
        self.user_token = Some(token);
    }

    pub fn get_user_token(&self) -> Option<HANDLE> {
        self.user_token
    }
}
