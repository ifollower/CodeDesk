...
// Remove only CodeDesk from all permissions: tccutil reset All com.codedesk.remote
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.codedesk.remote", &bundleid);
    }
    s = s.replace("codedesk", &crate::get_app_name().to_lowercase());
    s = s.replace("CodeDesk", &crate::get_app_name());
    s
}
